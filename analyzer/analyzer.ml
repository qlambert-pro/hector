...
let analyze_each_blk_new lbl_list rr_ops_list prog iifunc1 all_def func_name init_lbl_list errblks_list =
...
let analyze_def full_file all_fn = function
  ...
  let updated_blk_list =
    recheck_blks func_name iifunc1 lbl_list prog new_errblks_list in
  let updated_blk_list =
  ...
  let _ =
    match (updated_blk_list, rr_ops_list_new) with
      (h1::h2::t, l::u) -> analyze_each_blk_new lbl_list rr_ops_list_new prog iifunc1 !all_def func_name init_lbl_list updated_blk_list
    | _ -> []
  in
