...
let rec generate_report_new func_name  line_no iifunc1 = function
  []-> false
    ...
    |   Ast_c.ExprStatement (Some (((Ast_c.FunCall  ((((Ast_c.Ident (Ast_c.RegularName (id, [ii2]))), typ1), ii1), es)), typ), ii)) ->
           ...
           let st_lineno = Def.find_startline_no [h] in
            ...
            let ro = id in
               ...
               print_string("]]");
               print_string("\n");
               generate_report_new  func_name line_no iifunc1 t
    |  _-> generate_report_new  func_name line_no iifunc1 t
